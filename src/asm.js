...
function assemble (sourceCode, symbolDefs) {
  symtab = symbolDefs || {}
  codesrc = (sourceCode + '\n\n').split('\n\r')
  listing = ''
  var result = null
	...
	while (sym) {
		...
		if (c1=='.') {
			...
			if (sym.length==2) {
				if (pass==2) {
					...
					if (pragma=='.WORD') {
						code[code.length]=hi;
						listing += ' $'+getHexWord(v)+'            '+getHexByte(lo)+' '+getHexByte(hi);
					}
					else {
						listing += ' $'+getHexByte(lo)+'              '+getHexByte(lo);
					}
				}
				else {
					listing+=' '+sym[1];
		...
		if ((c1<'A') || (c1>'Z')) {
			...
			if (typeof addr=='undefined') {
				...
				var b2=addr.length;
				if (addr=='A') {
					if (pass==2) { listing+='A'; padd=1; }
					mode=1;
				}
				else if (a1=='#') {
					if (pass==2) { listing+=a1; padd=1; }
					b1=1;
					mode=2;
				}
				else if (a1=='*') {
				...
				if (pass==2) {
					...
					// compile
					for (var i=padd; i<12; i++) listing+=' ';
					listing+=getHexByte(instr);
					if (mode>1) {
						var op=oper&0xff;
						code[code.length]=op;
						listing+=' '+getHexByte(op);
						if (steptab[mode]>2) {
